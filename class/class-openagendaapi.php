...
class OpenAgendaApi
{
    ...
    public function openwp_basic_html( $openwp_data, $lang, $instance )
    {
        ...
        foreach ( $openwp_data['events'] as $events ) {
            ...
            if ( 'yes' === $instance['event-title'] && !empty($events['title'][$lang]) ) {
                ?>
            ...
            if ( 'yes' === $instance['event-description'] && !empty($events['description'][$lang]) ) {
                ?>
                ...
                ?>
                        <p class="openwp-event-description"><?php 
                echo  $parsedown->text( esc_textarea( $events['description']['en'] ) ) ;
                ?></p>
						<?php 
            }
    ...
    public function check_api()
    {
        ...
        if ( null === $check ) {
            ...
            <div class="notice notice-error openagenda-notice">
                <p><?php 
            _e( 'Woot! Your API Key seems to be non valid', 'wp-openagenda' );
            ?></p>
                <p><?php 
            printf( __( '<a href="%s" target="_blank">Find help</a>', 'wp-openagenda' ), esc_url( 'https://thivinfo.com/docs/openagenda-pour-wordpress/' ) );
            ?></p>
            </div>
			<?php 
        } else {
            ?>
            <div class="notice notice-success openagenda-notice"><?php 
            _e( 'OpenAgenda API Key valid', 'wp-openagenda' );
            ?></div>
			<?php 
        }
